...
roads= []
# start road
roads.append(xodr.create_road([xodr.Spiral(-0.004,0.00001,100), xodr.Spiral(0.00001,0.005,50), xodr.Arc(0.005,50)],id =0,left_lanes=3,right_lanes=4))
# intermittent road
roads.append(xodr.create_road([xodr.Spiral(0.0001,0.003,65), xodr.Spiral(0.003,0.00001,50) ],id =1,left_lanes=3,right_lanes=3))


#exit road
roads.append(xodr.create_road(xodr.Line(50),id =2,left_lanes=0,right_lanes=1))
# junctions for exit
roads.append(xodr.create_road(xodr.Spiral(0.005,0.0001,50),id =3,left_lanes=3,right_lanes=3,road_type=1)) # continue
roads.append(xodr.create_road(xodr.Spiral(0.005,-0.02,100),id =4,left_lanes=0,right_lanes=1,road_type=1)) # exit

# final road
roads.append(xodr.create_road([xodr.Spiral(-0.00001,-0.003,45),xodr.Arc(-0.003,60)],id =5,left_lanes=2,right_lanes=3))

# entry junction
roads.append(xodr.create_road([xodr.Line(30) ],id =6,left_lanes=2,right_lanes=3,road_type=2)) # continue
roads.append(xodr.create_road([xodr.Spiral(0.004,0.000001,50) ],id =7,left_lanes=1,right_lanes=0,road_type=2)) # entry
...
roads[4].add_predecessor(xodr.ElementType.road,0,xodr.ContactPoint.end,lane_offset=-3)
